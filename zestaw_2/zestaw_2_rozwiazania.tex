...
\def\showsolutions{} % comment this line to hide solutions, uncomment to show
...
\begin{align*}
    \log p = -\frac{1}{2} \log 2\pi + \left(N + \frac{1}{2}\right) \log N - 
    \frac{1}{2}\left(N + 1\right) \log \left[\left(\frac{N}{2} + z\right) 
    \left(\frac{N}{2} - z\right) \right] - z \log \frac{N/2 + z}{N/2 - z} - N \log 2
\end{align*}
...
\begin{align*}
\log p \approx -\frac{1}{2} \log 2\pi + \left(N + \frac{1}{2}\right) \log N \\
         - \frac{N+1}{2} \left[ 2\log(\frac{N}{2}) + \frac{2z}{N} - \frac{2z}{N} \right] \\
         - z \left[\frac{2z}{N} + \frac{2z}{N} \right] \\
         - N \log 2
\end{align*}

Powyższe rozwinięcie zawieraj jednak subtelny błąd: 
drugi człon jest rozwinięty do rzędu $z$, a trzeci do rzędu $z^2$.
Rozwijając sumę wyrażeń musimy zadbać o to, żeby oba 
były rozwinięte do tego samego rzędu, czyli do $z^2$.
Rozwijając $\log(\frac{N}{2} + z)$ do rzędu $z^2$:
\begin{align*}
    \log(\frac{N}{2} \pm z) = \log \frac{N}{2} + \log(1 \pm \frac{2z}{N}) \approx 
    \log \frac{N}{2} \pm \frac{2z}{N} - \frac{2z^2}{N^2}
\end{align*}

Uwzględniamy to w rozwinięciu:

\begin{align*}
          \log p \simeq -\frac{1}{2} \log 2\pi + \left(N + \frac{1}{2}\right) \log N \\
          - (N+1) \left[ \log(N) - \log(2) - \frac{2z^2}{N^2} \right] \\
          - \frac{4z^2}{N} \\
          - N \log 2 = \\
          -\frac{1}{2} \log 2\pi - \frac{1}{2} \log N + \log 2 - \frac{2z^2}{N} + \frac{2z^{2}}{N^2}= \\
          -\frac{1}{2} \log 2\pi - \frac{1}{2} \log N + \frac{1}{2} \log 2^{2} - \frac{2z^2}{N} + \frac{2z^{2}}{N^2} = \\
          -\frac{1}{2} \log \left( \frac{4}{2\pi N} \right) - \frac{2z^2}{N} + \frac{2z^{2}}{N^2}
\end{align*}

Wyraz $\frac{2z^2}{N^2}$ jest rzędu $1/N$ względem $\frac{2z^2}{N}$, więc możemy 
go pominąć.
...
Położenie to suma $N$ niezależnych zmiennych losowych $x_{i}$, $i \in \{1,2,\ldots,N\}$ 
o wartościach $\pm a$ przyjmowanych z prawdopodobieństwami $1/2$.
Średnia wartość jednego kroku:
\begin{align*}
    <x_{i}> = \frac{1/2a - 1/2a}{1/2 + 1/2} = 0
\end{align*}

Wariancja jednego kroku to:
\begin{align*}
    V(x_{i}) =  \frac{1}{2} (a - 0)^2 + \frac{1}{2} (-a - 0)^2 = a^2
\end{align*}
...
\begin{align*}
    \mu = N \langle x_{i} \rangle = N \cdot 0 = 0 \\
    \sigma^2 = N \sigma_{x_{i}}^{2} = N a^2
\end{align*}
...
(czynnik $\frac{1}{2\pi}$ można pominąć, bo i tak będzie po obu stronach równania 
i się skróci)
...
Oznaczmy ułamek pojazdów, które są ciężarówkami jako $P_C$, a 
ułamek samochodów osobowych jako $P_S$, inaczej ułamek cząstek 
w stanie $C$ i $S$:
\begin{align*}
    P = \begin{bmatrix}
        P_C \\
...
Ewolucja rozkładu pojazdów to $P(n+1) = \hat{p}P(n)$, gdzie $n$ to kolejne
kroki czasowe.
Interesuje nas rozkład stacjonarny, czyli taki, który nie zmienia się w czasie,
więc jest wektorem własnym macierzy $\hat{p}$ z wartością własną 1, czyli:
\begin{align*}
...
Zanim zaczniemy poszukiwać rozwiązania, sprawdźmy czy macierz $\hat{p}$ 
ma wartość własną równą 1, czyli czy istnieje rozkład stacjonarny:
\begin{align*}
    \det(\hat{p} - \lambda I) = 0 \\
    \det\begin{bmatrix}
        \frac{1}{4} - \lambda & \frac{1}{5} \\
        \frac{3}{4} & \frac{4}{5} - \lambda
    \end{bmatrix} = 0 \\
    (\frac{1}{4} - \lambda)(\frac{4}{5} - \lambda) - \frac{3}{20} = 0 \\
    \lambda^2 - \frac{21}{20} \lambda + \frac{1}{20} = 0 \\
    (\lambda - 1)(\lambda - \frac{1}{20}) = 0
\end{align*}

Macierz $\hat{p}$ ma wartość własną równą 1, więc istnieje rozkład stacjonarny.
Poszukujemy wektora własnego odpowiadającego tej wartości własnej.
Do warunku stacjonarności dodajemy warunek normalizacji $P_C^{eq} + P_S^{eq} = 1$.
...
są takie same. Oznaczmy $P(n)^{eq}$ jako prawdopodobieństwo znalezienia cząstki 
w stanie $n$ w stanie stacjonarnym. Mamy więc:
\begin{align*}
    P^{eq}(n) p(n \to m) = P^{eq}(m) p(m \to n)
\end{align*}

Dla stanu brzegowego i następnych mamy:
\begin{align*}
...
Sumując szereg trzeba pamiętać, że wyraz 0 ma inną postać niż pozostałe,
więc sumujemy od 1 do nieskończoności i potem dodajemy $P^{eq}(0)$:
...
Otrzymaliśmy rozkład typu $p^{n}$, $p<1$, więc jest to rozkład wykładniczy:
\begin{align*}
    P^{eq}(n) = \frac{1-2p}{2} e^{n \log \frac{p}{1-p}}
\end{align*}
...
Dążymy do uzyskania pochodnych, więc przypomnijmy sobie numeryczne przybliżenia pochodnych, które
otrzymujemy z rozwinięcia Taylora:

\begin{align*}
    P(x - \delta x, t) = P(x, t) - \frac{\partial P}{\partial x} \delta x + \frac{1}{2} \frac{\partial^2 P}{\partial x^2} (\delta x)^2 + \ldots \\
    P(x + \delta x, t) = P(x, t) + \frac{\partial P}{\partial x} \delta x + \frac{1}{2} \frac{\partial^2 P}{\partial x^2} (\delta x)^2 + \ldots \\
\end{align*}

Dodajemy stronami:
\begin{align*}
    P(x - \delta x, t) + P(x + \delta x, t) = 2P(x, t) + \frac{\partial^2 P}{\partial x^2} (\delta x)^2 + \ldots \\
    P(x- \delta x, t) - P(x + \delta x, t) - 2P(x,t) = \frac{\partial^2 P}{\partial x^2} (\delta x)^2 + \ldots \\
    \frac{\partial^2 P}{\partial x^2} = \frac{P(x- \delta x, t) - 2P(x,t) + P(x + \delta x, t)}{(\delta x)^2} + {\cal O}(\delta x) \\
\end{align*}

Odejmujemy stronami:
\begin{align*}
    P(x + \delta x, t) - P(x - \delta x, t) = 2 \frac{\partial P}{\partial x} \delta x + {\cal O}(\delta x^2) \\
    \frac{\partial P}{\partial x} = \frac{P(x + \delta x, t) - P(x - \delta x, t)}{2 \delta x} + {\cal O}(\delta x)
\end{align*}



Brakuje nam $P(x, t)$ i malej w mianowniku, więc odejmujemy od obu stron $P(x, t)$ i dzielimy 
przez $\delta t$:
\begin{align*}
...
\begin{align*}
    P^{eq}(x \to 0) = P_{0} \rightarrow A + B = P_{0} \\
    P^{eq}(x \to \infty) = 0 \rightarrow B = 0 \to A = P_{0}
\end{align*}

Finalnie:
\begin{align*}
    P^{eq}(x) = P_{0} e^{-\frac{Ux}{D}}
\end{align*}

Sprawdźmy czy rozkład jest normalizowalny:
\begin{align*}
    \int_{0}^{\infty} P^{eq}(x) dx = \int_{0}^{\infty} P_{0} e^{-\frac{Ux}{D}} dx = \frac{P_{0} D}{U} \\
    P_{0} = \frac{U}{D}
\end{align*}
Ostatecznie rozkład równowagowy to:
\begin{align*}
    P^{eq}(x) = \frac{U}{D} e^{-\frac{Ux}{D}}
\end{align*}
...
\begin{align*}
    [\alpha] = \frac{1}{m} \\
    [D] = \frac{m^{2}}{s} \\
    [U] = \frac{[\alpha] m^{2}}{s} = \frac{m}{s} \\
    [P] = [\frac{U}{D}] = \frac{1}{m}
\end{align*}

Rozkład prawdopodobieństwa, $P^{eq}$ ma jednostkę ponieważ to jest 
