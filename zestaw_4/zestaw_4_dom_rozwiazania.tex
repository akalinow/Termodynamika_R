...
szerokości kątowej $\Delta\Theta$.
...
Średnia prędkość $v_p$ neutronów o temperaturze $T$:
...
\begin{align*}
  ...
  = 4\pi \left( \frac{m}{2\pi k_B T} \right)^{3/2} 
    ...
    \sqrt{\frac{8 \cdot 1.38 \cdot 10^{-23} \cdot 300}{\pi \cdot 1.67 \cdot 10^{-27}}} 
    = 2512\,\text{m/s}
\end{align*}
...
= \frac{0.25 \cdot 2512}{0.8} = 785\,\text{rad/s} = 125\,\text{Hz}
\end{align*}

Zakres prędkości przepuszczanych przez szczelinę o szerokości $\Delta\Theta$:
\begin{align*}
\Delta \phi = \Delta\Theta / L \implies \Delta v = \frac{\Delta\Theta v}{\Theta} = \frac{0.02 \cdot 2512}{0.25} = 201\,\text{m/s}
\end{align*}

Ułamek neutronów o temperaturze $T$ przepuszczanych przez selektor:
\begin{align*}
r = \int_{v_p - \Delta v/2}^{v_p + \Delta v/2} f(v) dv = 
4\pi \left( \frac{m}{2\pi k_B T} \right)^{3/2} 
\int_{v_p - \Delta v/2}^{v_p + \Delta v/2} v^2 \exp\left( -\frac{m v^2}{2 k_B T} \right) dv \approx 0.07
\end{align*}
...
v_p = maxwell.mean(scale=scale)
delta_v = 201
v_min = v_p - delta_v / 2
v_max = v_p + delta_v / 2
r = maxwell.cdf(v_max, scale=scale) - maxwell.cdf(v_min, scale=scale)
print("Ratio = {:3.2f}".format(r))
\end{verbatim}
...
\begin{itemize}
  ...
  \item energię kinetyczną wymaganą by neutrony zbliżyły się do siebie na 
...
Średnia prędkość protonów w Słońcu:
\begin{align*}
\langle v \rangle = \sqrt{\frac{8 k_B T}{\pi m}} = 
\sqrt{\frac{8 \cdot 1.38 \cdot 10^{-23} \cdot 1.5 \cdot 10^7}{\pi \cdot 1.67 \cdot 10^{-27}}} = 
5.62 \cdot 10^5\,\text{m/s}
\end{align*}

Stąd średnia energia kinetyczna protonów w Słońcu:
\begin{align*}
\langle E_k \rangle = \frac{1}{2} m \langle v \rangle^2 = 
\frac{1}{2} \cdot 1.67 \cdot 10^{-27} \cdot (5.62 \cdot 10^5)^2 = 
2.7 \cdot 10^{-13}\,\text{J} = 1.6\,\text{keV}
\end{align*}
...
\begin{align*}
r = \int_{v_0}^\infty f(v) dv = 
4\pi \left( \frac{m}{2\pi k_B T} \right)^{3/2} 
\int_{v_0}^\infty v^2 \exp\left( -\frac{m v^2}{2 k_B T} \right) dv
\end{align*} 
...
\frac{V_0}{k_B T} = \frac{1800~\text{keV}}{1.3~\text{keV}} = 1400 \to
\exp\left( -\frac{V_0}{k_B T} \right) \simeq 10^{-608}
\end{align*}

Całkę można więc oszacować zachowując jedynie pierwszy wyraz:
\begin{align*}
\int_{v_0}^\infty v^2 \exp\left( -\frac{m v^2}{2 k_B T} \right) dv \simeq
\frac{v}{2}\frac{2k_B T}{m} \exp\left( -\frac{m v^2}{2 k_B T} \right) \Bigg|_{v_0}^\infty =
\frac{v_0}{2}\frac{k_B T}{m} \exp\left( -\frac{m v_0^2}{k_B T} \right)
\end{align*}

Ostatecznie:
\begin{align*}
r \simeq 4\pi \left( \frac{m}{2\pi k_B T} \right)^{3/2} \cdot \frac{v_0}{2}\frac{2k_B T}{m} \exp\left( -\frac{m v_0^2}{2 k_B T} \right) = \\
\frac{4\pi}{(2\pi)^{3/2}}\left( \frac{m}{2\pi k_B T} \right)^{1/2} \frac{v_0}{2} \exp\left( -\frac{V_0}{k_B T} \right) 
\end{align*}

Więc ułamek protonów w Słońcu, które mają wystarczającą energię do zbliżenia się na odległość 
rzędu rozmiaru neutronu jest rzędu $10^{-608}$...
